...
class Pix2PixMSModel(BaseModel):
    def name(self):
        return 'Pix2PixMSModel'
    ...
    def backward_G(self):
        ...
        # Feature Matching
        if self.opt.use_feature_matching:
            real_feature = self.netFM(self.real_A).detach()
            real_feature.requires_grad = False
            self.loss_G_FM = torch.nn.MSELoss()(self.netFM(self.fake_B), real_feature) * self.opt.lambda_FM
        else:
            self.loss_G_FM = 0
